...
namespace MIM40kFactions
{
    public class CompAbilityEffect_ShootBeam : CompAbilityEffect
    {
        ...
        public override void DrawEffectPreview(LocalTargetInfo target)
        {
            ...
            {
                GenDraw.DrawFieldEdges(tmpHighlightCells.ToList(), Props.highlightColor ?? Color.white);
            }
            if (tmpSecondaryHighlightCells.Any())
            {
                GenDraw.DrawFieldEdges(tmpSecondaryHighlightCells.ToList(), Props.secondaryHighlightColor ?? Color.white);
            }
