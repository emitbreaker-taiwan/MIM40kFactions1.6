...
{
    public class SanitizerApparelTracker : ISanitizerTracker, IExposable
    {
        private HashSet<string> data = new HashSet<string>(); // Format: "pawnID-thingID"

        public bool IsAlreadyApplied(Pawn pawn, object source)
        {
            if (!(source is Thing thing))
            {
                Log.Warning($"[Sanitizer] ApparelTracker received invalid source: {source?.GetType().Name}");
                return false;
            }
        ...
        public void Register(Pawn pawn, object source)
        {
            if (!(source is Thing thing))
            {
                Log.Warning($"[Sanitizer] ApparelTracker tried to register invalid source: {source?.GetType().Name}");
                return;
            }
        ...
        private string Key(Pawn p, Thing t) => $"{p.ThingID}-{t.ThingID}";
        ...
        {
            Scribe_Collections.Look(ref data, "sanitizerApparelTracker", LookMode.Value);
        }
    }
}
