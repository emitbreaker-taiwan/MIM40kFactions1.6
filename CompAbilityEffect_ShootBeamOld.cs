...
{
    public class CompAbilityEffect_ShootBeamOld : CompAbilityEffect
    {
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            // Create the mote BEFORE validation check
            if (Props.beamMoteDef != null && mote == null)
            {
                mote = MoteMaker.MakeInteractionOverlay(Props.beamMoteDef, Pawn, new TargetInfo(path[0].ToIntVec3(), Pawn.Map));
            }

            // Add debug output if in dev mode
            if (Prefs.DevMode && !Utility_TargetValidator.IsValidTargetForAbility(target.Thing, Pawn, Props))
            {
                Log.Message($"[MIM Debug] Beam ability rejected target: {target.Thing?.Label ?? "null"}");
                // We still want to handle empty space targeting - only return if there's an actual thing targeted
            ...
            if (BeamVerb.state == VerbState.Bursting)
            {
                ...
                else
                {
                    ...
                    int shotsToFire = burstShotsLeft;
                    for (int i = 0; i < shotsToFire; i++)
                    {
                        if (Props.beamMoteDef != null && (mote == null || mote.Destroyed))
                        {
        ...
        private bool TryCastShot(LocalTargetInfo target)
        {
            ...
            {
                if (Prefs.DevMode)
                    Log.Message("Invalid target for beam shot");
                return false;
            }
