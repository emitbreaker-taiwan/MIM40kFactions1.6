...
namespace MIM40kFactions
{
    public class PawnRenderNodeWorker_HeadAttachmentCustom : PawnRenderNodeWorker_FlipWhenCrawling
    {
        ...
        public override Vector3 ScaleFor(PawnRenderNode node, PawnDrawParms parms)
        {
            ...
            if (node.AnimationWorker != null && node.AnimationWorker.Enabled(parms.pawn.Drawer.renderer.CurAnimation, node, null, parms) && !parms.flags.FlagSet(PawnRenderFlags.Portrait))
            {
                vector = vector.MultipliedBy(node.AnimationWorker.ScaleAtTick(node.tree.AnimationTick, parms.pawn.Drawer.renderer.CurAnimation, node, null, parms));
            }
